  ...
  var Notes = {
    previewNote() {
      if (window.File && window.FileList && window.FileReader) {
        ...
        const fileInput = document.getElementById('note_image');
        ...
        if (dropZone) {
          ...
          dropZone.addEventListener('drop', (e) => {
            ...
            // if on note/id/edit hide preview image on drop
            if (previewImage) {
              previewImage.style.display = 'none';
            }
            // If on notes/new hide dropzone on drop
            if(newNoteForm) {
    ...
    noteHover() {
      $('.note').hover(function() {
        $(this).children('.note-data').toggleClass('visible');
      });
    }
